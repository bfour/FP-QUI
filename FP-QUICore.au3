...
#cs ----------------------------------------------------------------------------
 ...
 Invokes for new notifications can be send via a named pipe, using the same command line sytanx as for the executable itself.
...
Func _start()
	...
	;if another instance is already up and running forward the CmdLine via pipe
	If _Singleton("FP-QUICore", 1) == 0 Then ; pipe create failed (another instance is likely to be existent)
