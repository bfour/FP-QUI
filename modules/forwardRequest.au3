...
#cs
   this forwards a request (via cmdLine) to an existing instance of FP-QUI,
   that runs the "FP-QUI"-pipe (since only one instance of such a pipe is allowed,
   there can only be one process) (this instance is also referred to as "main"
   (--> forward to main))

   -) checks if request is an update for an existing notification
     (indicated by winHandle<>"" in arguments)
      -) if true
