...
call :SendMessageViaCmdLineXml
call :SendMessageViaStdInPipe
goto :EOF

:SendMessageViaCmdLine
fpquitip text "%date%-%time% - fpquitip flat - CmdLine - Ok." delay 15000 trans 255  font "Courier New Bold" fontSize 16 bkColor green
goto :EOF

:SendMessageViaCmdLineXml
fpquitip xml "<text>%date%-%time% - fpquitip xml - CmdLine - Ok.</text> <delay>15000</delay> <trans>255</trans> <font>Courier New Bold</font> <fontSize>16</fontSize> <bkColor>green</bkColor>"
goto :EOF

:SendMessageViaStdInPipe
set message=""
call :Concat message %message% "<text>%date%-%time% - fpquitip - pipe - Ok.</text>"
...
%ComSpec% /c echo xml %message% | fpquitip
goto :EOF
